exports.plugin = {
  ...
  register: async (server, options) => {
    ...
    const { getResources, addFoto } = server.methods;
    server.route([
      {
        method: 'POST',
        path: '/crear_reporte',
        ...
        handler: async (request, h) => {
          ...
          return saveReporte.modifiedCount === 1 ? h.response({ message: 'reporte creado' }) : h.notCreated();
        },
      },
      {
        method: 'GET',
        path: '/verReporte/{cotizacionId}/{proyectoId}',
        options: {
        ...
        handler: async (request, h) => {
          ...
          const pipeline = [{ $match: query }, { $project: { _id: 0, reporte: 1 } }];
          const reportes = await getResources(COL_PRY, pipeline);
          return reportes[0].reporte.length > 0 ? h.response(reportes) : h.notFound('NO_EXISTEN_REPORTES');
        },
      },
    ]);
  },
