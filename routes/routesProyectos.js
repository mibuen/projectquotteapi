exports.plugin = {
  ...
  register: async (server, options) => {
    ...
    const { getResources, getResource, saveMany, updateResource, addFoto } =
      server.methods
    ...
    server.route([
      ...
      },
      {
        ...
        handler: async (request, h) => {
          ...
          for (
            let index = 1;
            index <= parseInt(cotizacion.sitios, 10);
            index++
          ) {
            proyectos.push({
      ...
      },
      {
        ...
        handler: async (request, h) => {
          ...
          const proyecto = await updateResource(
            colPry,
      ...
      },
      {
        ...
        handler: async (request, h) => {
          ...
          const { cotizacionId, proyectoId, tipo, laFoto, reporte } =
            request.payload
